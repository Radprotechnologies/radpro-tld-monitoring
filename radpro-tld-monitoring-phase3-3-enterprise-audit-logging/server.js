...
const DB_DRIVER = (process.env.DB_DRIVER || process.env.DATABASE_DRIVER || 'sqlite').toLowerCase();
let DatabaseSync;
let createPostgresSyncBridge;
if (DB_DRIVER === 'postgres' || DB_DRIVER === 'postgresql') {
  ({ createPostgresSyncBridge } = require('./scripts/pg-sync-bridge'));
} else {
  try {
    ({ DatabaseSync } = require('node:sqlite'));
  } catch (error) {
    console.error('\nRadpro TLD Monitoring requires Node.js 22.5+ for this Windows-friendly SQLite build.');
    console.error('Install/update Node.js 22 LTS or newer, then run npm install and npm start again.');
    console.error('Technical detail:', error && error.message ? error.message : error);
    process.exit(1);
  }
}
...
const DB_FILE = path.resolve(process.env.DB_FILE || path.join(DATA_DIR, 'radpro_tld.db'));
...
const db = (DB_DRIVER === 'postgres' || DB_DRIVER === 'postgresql')
  ? createPostgresSyncBridge()
  : new DatabaseSync(DB_FILE);
// Compatibility helpers: the original app used better-sqlite3. Node 22's built-in
// node:sqlite API is synchronous and similar, but does not expose pragma() or
// transaction() helpers, so we provide small equivalents here. In PostgreSQL mode,
// scripts/pg-sync-bridge provides the same prepare/get/all/run surface for the
// existing Express routes so Phase 2B can run without a full route rewrite.
if (!(DB_DRIVER === 'postgres' || DB_DRIVER === 'postgresql')) {
...
console.log(`Radpro TLD database driver: ${DB_DRIVER}`);
...
[
  ...
  ['audit_logs', 'actorRole', "TEXT DEFAULT ''"], ['audit_logs', 'organizationId', "TEXT DEFAULT ''"], ['audit_logs', 'hospitalId', "TEXT DEFAULT ''"],
  ['audit_logs', 'module', "TEXT DEFAULT ''"], ['audit_logs', 'eventType', "TEXT DEFAULT ''"], ['audit_logs', 'entityType', "TEXT DEFAULT ''"], ['audit_logs', 'entityId', "TEXT DEFAULT ''"],
  ['audit_logs', 'oldValue', "TEXT DEFAULT ''"], ['audit_logs', 'newValue', "TEXT DEFAULT ''"], ['audit_logs', 'metadata', "TEXT DEFAULT ''"],
  ['audit_logs', 'ipAddress', "TEXT DEFAULT ''"], ['audit_logs', 'userAgent', "TEXT DEFAULT ''"], ['audit_logs', 'httpMethod', "TEXT DEFAULT ''"], ['audit_logs', 'requestPath', "TEXT DEFAULT ''"],
  ['audit_logs', 'statusCode', 'INTEGER NOT NULL DEFAULT 200'], ['audit_logs', 'success', 'INTEGER NOT NULL DEFAULT 1'], ['audit_logs', 'requestId', "TEXT DEFAULT ''"]
...
function safeAuditJson(value, maxLength = 6000) {
  const clean = (input, depth = 0) => {
    if (depth > 4) return '[depth-limited]';
    if (Array.isArray(input)) return input.slice(0, 50).map(v => clean(v, depth + 1));
    if (input && typeof input === 'object') {
      const out = {};
      Object.entries(input).slice(0, 100).forEach(([k, v]) => {
        out[k] = /password|secret|token|signature|codeHash/i.test(k) ? '[REDACTED]' : clean(v, depth + 1);
      });
      return out;
    }
    if (typeof input === 'string' && input.length > 1000) return `${input.slice(0, 1000)}…`;
    return input;
  };
  try { const text = JSON.stringify(clean(value)); return text.length > maxLength ? `${text.slice(0, maxLength)}…` : text; }
  catch (_) { return ''; }
}
function clientIp(req) { return String(req?.headers?.['x-forwarded-for'] || '').split(',')[0].trim() || String(req?.ip || req?.socket?.remoteAddress || ''); }
function inferAuditModule(p = '') {
  p=String(p).toLowerCase();
  if(p.includes('/auth/')) return 'Authentication'; if(p.includes('/tenancy')||p.includes('/organizations')||p.includes('/hospitals')) return 'Organization & Hospital';
  if(p.includes('/employees')||p.includes('/departments')) return 'Employee Management'; if(p.includes('/doses')||p.includes('/acknowledgements')||p.includes('/awareness')) return 'TLD Dose & Acknowledgement';
  if(p.includes('/training')) return 'Radiation Safety Training'; if(p.includes('/attachments')||p.includes('/documents')) return 'Document Management'; if(p.includes('/rsc/')) return 'Radiation Safety Committee';
  if(p.includes('/overexposure')||p.includes('/investigations')) return 'Overexposure & Investigation'; if(p.includes('/backup')) return 'Backup & Restore'; if(p.includes('/settings')) return 'Settings';
  if(p.includes('/notifications')||p.includes('/reminders')) return 'Notifications & Reminders'; return 'System';
}
function writeAudit(e={}) {
  db.prepare(`INSERT INTO audit_logs (actorId,actorName,actorRole,organizationId,hospitalId,module,eventType,action,details,entityType,entityId,oldValue,newValue,metadata,ipAddress,userAgent,httpMethod,requestPath,statusCode,success,requestId,createdAt) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    e.actorId||null,e.actorName||'System',e.actorRole||'',e.organizationId||'',e.hospitalId||'',e.module||'System',e.eventType||'SYSTEM',e.action||'System event',e.details||'',e.entityType||'',e.entityId||'',e.oldValue||'',e.newValue||'',e.metadata||'',e.ipAddress||'',e.userAgent||'',e.httpMethod||'',e.requestPath||'',Number(e.statusCode||200),e.success===false?0:1,e.requestId||'',e.createdAt||nowISO());
}
function logAudit(req, action, details='', options={}) {
  const actor=req?.session?.user||req?.user||null; if(req) req._enterpriseAuditLogged=true;
  writeAudit({actorId:actor?.id,actorName:actor?.name||actor?.username||'System',actorRole:accessRoleFor(actor||{}),organizationId:actor?.organizationId||'',hospitalId:actor?.hospitalId||'',module:options.module||inferAuditModule(req?.originalUrl||''),eventType:options.eventType||'USER_ACTION',action,details,entityType:options.entityType||'',entityId:options.entityId||'',oldValue:options.oldValue?safeAuditJson(options.oldValue):'',newValue:options.newValue?safeAuditJson(options.newValue):'',metadata:options.metadata?safeAuditJson(options.metadata):'',ipAddress:clientIp(req),userAgent:String(req?.headers?.['user-agent']||''),httpMethod:String(req?.method||''),requestPath:String(req?.originalUrl||''),statusCode:options.statusCode||200,success:options.success!==false,requestId:req?._auditRequestId||''});
}
function logSystem(action, details='', options={}) { writeAudit({actorName:'System',actorRole:'system',module:options.module||'System',eventType:options.eventType||'SYSTEM',action,details,metadata:options.metadata?safeAuditJson(options.metadata):'',ipAddress:options.ipAddress||'',userAgent:options.userAgent||'',httpMethod:options.httpMethod||'',requestPath:options.requestPath||'',statusCode:options.statusCode||200,success:options.success!==false,requestId:options.requestId||''}); }
...
app.use((req,res,next)=>{
  req._auditRequestId=crypto.randomUUID?crypto.randomUUID():makeId('req'); const started=Date.now();
  res.on('finish',()=>{ try { const method=String(req.method).toUpperCase(), url=String(req.originalUrl||'');
    if(!['POST','PUT','PATCH','DELETE'].includes(method)||!url.startsWith('/api/')||url.startsWith('/api/audit')||req._enterpriseAuditLogged) return;
    const actor=req.session?.user||req.user||null;
    writeAudit({actorId:actor?.id,actorName:actor?.name||actor?.username||'Anonymous',actorRole:actor?accessRoleFor(actor):'anonymous',organizationId:actor?.organizationId||'',hospitalId:actor?.hospitalId||'',module:inferAuditModule(url),eventType:res.statusCode>=400?'FAILED_ACTION':'API_MUTATION',action:`${method} ${url.split('?')[0]}`,details:`${res.statusCode} · ${Date.now()-started} ms`,newValue:safeAuditJson(req.body||{}),metadata:safeAuditJson({query:req.query||{},durationMs:Date.now()-started}),ipAddress:clientIp(req),userAgent:String(req.headers['user-agent']||''),httpMethod:method,requestPath:url,statusCode:res.statusCode,success:res.statusCode<400,requestId:req._auditRequestId});
  } catch(e){console.error('Enterprise audit fallback failed:',e.message);} }); next();
});
...
function getCloudStabilityStatus() {
  ...
  const dbBytes = safeFileSize(DB_FILE);
  ...
  return {
    ...
    phase: DB_DRIVER === 'sqlite' ? 'Phase 1 cloud-stabilized SQLite build' : 'Phase 2B PostgreSQL runtime adapter build',
    nodeEnv: NODE_ENV,
    nodeVersion: process.version,
    time: nowISO(),
    paths: { dataDir: DATA_DIR, dbFile: DB_FILE, uploadDir: UPLOAD_DIR },
    storage: {
      dataDirWritable: dataWrite.ok,
      uploadDirWritable: uploadWrite.ok,
      dbExists: fs.existsSync(DB_FILE),
      dbSizeMB: Number((dbBytes / (1024 * 1024)).toFixed(3)),
...
app.post('/api/auth/login', async (req, res) => {
  ...
  if (!employee || !employee.enabled || !bcrypt.compareSync(password, employee.passwordHash)) {
    logSystem('Failed login', `Username: ${username || '(blank)'}`, { module:'Authentication', eventType:'FAILED_LOGIN', ipAddress:clientIp(req), userAgent:String(req.headers['user-agent']||''), httpMethod:req.method, requestPath:req.originalUrl, statusCode:401, success:false, requestId:req._auditRequestId, metadata:{username, reason:!employee?'user_not_found':(!employee.enabled?'disabled':'invalid_password')} });
    req._enterpriseAuditLogged = true;
  ...
  logAudit(req,'Login successful',user.username,{module:'Authentication',eventType:'LOGIN',entityType:'employee',entityId:user.id,metadata:{username:user.username,role:user.accessRole||user.role}});
  res.json({ user });
});
...
app.post('/api/auth/verify-2fa', (req, res) => {
  ...
  delete req.session.pending2FA;
  logAudit(req,'Login 2FA verified',req.session.user.username,{module:'Authentication',eventType:'LOGIN_2FA',entityType:'employee',entityId:req.session.user.id});
  res.json({ user: req.session.user });
});

app.post('/api/auth/logout', requireAuth, (req, res) => {
  logAudit(req,'Logout',req.user.username,{module:'Authentication',eventType:'LOGOUT',entityType:'employee',entityId:req.user.id});
...
app.post('/api/acknowledgements', requireAuth, (req, res) => {
  ...
  logAudit(req,'Dose e-signed acknowledgement',`${period||quarter} by ${signerName}`,{module:'TLD Dose & Acknowledgement',eventType:'DOSE_ACKNOWLEDGEMENT',entityType:'acknowledgement',entityId:existing?existing.id:'',newValue:{employeeId:req.user.id,quarter,period,signerName,signedAt}});
  res.json({ ok: true });
});
...
app.post('/api/awareness/acknowledge', requireAuth, (req, res) => {
  ...
  logAudit(req,'TLD awareness poster accepted',`${quarter} ${posterVersion}`,{module:'TLD Dose & Acknowledgement',eventType:'AWARENESS_ACKNOWLEDGEMENT',newValue:{employeeId:req.user.id,quarter,posterVersion,acknowledgedAt:ts}});
...
app.post('/api/training/submit', requireAuth, (req, res) => {
  ...
  logAudit(req,'Radiation safety training submitted',`${quiz.quarter}: ${score}/${total}`,{module:'Radiation Safety Training',eventType:'TRAINING_COMPLETION',entityType:'training_attempt',entityId:id,newValue:{quizId:quiz.id,quarter:quiz.quarter,score,totalQuestions:total,percentage,passed:percentage>=60}});
...
function auditQueryParts(req) {
  const where=[],params=[]; const q=String(req.query.q||'').trim(), mod=String(req.query.module||'').trim(), actor=String(req.query.actor||'').trim(), success=String(req.query.success||'').trim(), from=String(req.query.from||'').trim(), to=String(req.query.to||'').trim();
  if(q){const like=`%${q}%`;where.push('(LOWER(action) LIKE LOWER(?) OR LOWER(details) LIKE LOWER(?) OR LOWER(actorName) LIKE LOWER(?) OR LOWER(entityId) LIKE LOWER(?))');params.push(like,like,like,like);} if(mod){where.push('module=?');params.push(mod);} if(actor){where.push('LOWER(actorName) LIKE LOWER(?)');params.push(`%${actor}%`);} if(success==='true')where.push('success=1'); if(success==='false')where.push('success=0'); if(from){where.push('createdAt>=?');params.push(`${from}T00:00:00.000Z`);} if(to){where.push('createdAt<=?');params.push(`${to}T23:59:59.999Z`);}
  const role=accessRoleFor(req.user); if(role!=='sysadmin'){if(req.user.organizationId){where.push("(organizationId=? OR organizationId='')");params.push(req.user.organizationId);} if(['admin','rso','auditor'].includes(role)&&req.user.hospitalId){where.push("(hospitalId=? OR hospitalId='')");params.push(req.user.hospitalId);}}
  const page=Math.max(1,Number(req.query.page||1)),pageSize=Math.min(200,Math.max(10,Number(req.query.pageSize||50))); return {whereSql:where.length?` WHERE ${where.join(' AND ')}`:'',params,page,pageSize,offset:(page-1)*pageSize};
}
app.get('/api/audit',requireAuth,requireAuditAccess,(req,res)=>{const f=auditQueryParts(req);const total=Number(db.prepare(`SELECT COUNT(*) AS c FROM audit_logs${f.whereSql}`).get(...f.params)?.c||0);const rows=db.prepare(`SELECT * FROM audit_logs${f.whereSql} ORDER BY createdAt DESC,id DESC LIMIT ? OFFSET ?`).all(...f.params,f.pageSize,f.offset);const modules=db.prepare("SELECT DISTINCT module FROM audit_logs WHERE module<>'' ORDER BY module").all().map(r=>r.module);res.json({logs:rows,pagination:{page:f.page,pageSize:f.pageSize,total,pages:Math.max(1,Math.ceil(total/f.pageSize))},filters:{modules}});});
app.get('/api/audit/export.csv',requireAuth,requireAuditAccess,(req,res)=>{const f=auditQueryParts(req);const rows=db.prepare(`SELECT * FROM audit_logs${f.whereSql} ORDER BY createdAt DESC,id DESC`).all(...f.params);logAudit(req,'Audit log exported',`${rows.length} record(s)`,{module:'Audit & Compliance',eventType:'AUDIT_EXPORT',metadata:{filters:req.query,count:rows.length}});sendCsv(res,`radpro_tld_enterprise_audit_${new Date().toISOString().slice(0,10)}.csv`,['id','createdAt','actorName','actorRole','organizationId','hospitalId','module','eventType','action','details','entityType','entityId','ipAddress','httpMethod','requestPath','statusCode','success','requestId','oldValue','newValue','metadata'],rows);});

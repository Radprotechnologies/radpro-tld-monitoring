...
function currentAccessRole() {
  ...
  if (username === 'radpro' || clinicalRole.includes('radpro super admin') || clinicalRole.includes('radpro panel admin')) return 'sysadmin';
...
async function finishLogin(user) {
  ...
  if (canAccessAdmin()) {
    showView('admin');
    await loadAdminData();
    const targetTab = currentAccessRole() === 'sysadmin' || currentAccessRole() === 'org_admin' ? 'tabTenancy' : 'tabEmployees';
    $$('.admin-tab').find(btn => btn.dataset.tab === targetTab && !btn.classList.contains('hidden'))?.click();
  } else {
    showView('employee');
...
async function loadAdminData() {
  const [tenancy, employees, doses, attachments, settings, ack, periods, audit, investigations, overexposureCases, reminderLogs, notifications, rscMembers, rscMeetings, rscDocuments, trainingModule, trainingResults, trainingQuestions] = await Promise.all([
    api('/api/tenancy'),
    api('/api/employees'),
    api('/api/doses'),
    api('/api/attachments'),
    ...
    api('/api/doses/periods'),
    api('/api/audit'),
    api('/api/investigations'),
    api('/api/overexposure-cases'),
    api('/api/reminders/logs'),
    api('/api/notifications'),
    api('/api/rsc/members'),
    api('/api/rsc/meetings'),
    api('/api/rsc/documents'),
    api(`/api/training/module?quarter=${encodeURIComponent(currentQuarterLabel())}`),
    api(`/api/training/results?quarter=${encodeURIComponent(currentQuarterLabel())}`),
    api(`/api/training/questions?quarter=${encodeURIComponent(currentQuarterLabel())}`)
  ]);
...
async function loadEmployeeData() {
  const [doses, attachments, settings, ack, notifications, awareness, trainingModule, trainingQuiz] = await Promise.all([
    api('/api/doses'),
    api('/api/attachments'),
    api('/api/settings'),
    api('/api/acknowledgements'),
    api('/api/notifications'),
    api(`/api/awareness/status?quarter=${encodeURIComponent(currentQuarterLabel())}`),
    api(`/api/training/module?quarter=${encodeURIComponent(currentQuarterLabel())}`),
    api(`/api/training/quiz?quarter=${encodeURIComponent(currentQuarterLabel())}`)
  ]);
  state.doses = doses.doses || [];
  state.attachments = attachments.attachments || [];
  state.settings = settings.settings || {};
...
function renderTenancySection() {
  ...
  }).join('') || '<div class="text-slate-500">No organizations registered.</div>';
...
async function saveOrganization() {
  ...
  try {
    await api(id ? `/api/organizations/${encodeURIComponent(id)}` : '/api/organizations', { method: id ? 'PUT' : 'POST', body });
    resetOrganizationForm();
    setMsg('orgMsg', id ? 'Organization updated.' : 'Organization registered.', true);
    await loadAdminData();
...
async function saveHospital() {
  ...
  try {
    ...
    setMsg('hospitalMsg', (id ? 'Hospital / institute updated.' : 'Hospital / institute registered.') + accessMsg, true);
    await loadAdminData();
...
function renderRoleChrome() {
  ...
  setText('heroMetricTwoValue', isSysadmin ? String(state.hospitals.length) : 'SQLite');
  ...
  $$('.admin-tab').forEach((btn) => {
    if (isSysadmin) btn.classList.toggle('hidden', !sysadminTabs.has(btn.dataset.tab));
    else if (!isOrgAdmin && btn.dataset.tab === 'tabTenancy') btn.classList.add('hidden');
...
function renderDashboardCards() {
  if (currentAccessRole() === 'sysadmin') {
    ...
    $('cardCompleteQuarter').textContent = 'SQLite';
    return;
  }
...
function renderAnalytics() {
  ...
  const ctx = $('doseTrendChart')?.getContext('2d');
  if (ctx && window.Chart) {
...
function renderAuditLog() {
  ...
  if (!tbody) return;
  if (!state.audit.length) { tbody.innerHTML = emptyRow(4, 'No audit records yet.'); return; }
  tbody.innerHTML = state.audit.slice(0, 200).map(log => `
    <tr><td class="table-cell">${escapeHTML(fmtDate(log.createdAt))}</td><td class="table-cell">${escapeHTML(log.actorName || '')}</td><td class="table-cell">${escapeHTML(log.action || '')}</td><td class="table-cell">${escapeHTML(log.details || '')}</td></tr>
  `).join('');
}
...
function showAwarenessIfRequired() {
  ...
  $('awarenessPosterImg').src = aw.posterUrl || '/assets/tld_awareness_hindi.jpg';
  $('awarenessQuarterText').textContent = `Please read this TLD safe-use poster and acknowledge it for ${aw.quarter || currentQuarterLabel()}.`;
  $('awarenessConfirm').checked = false;
  $('awarenessMsg').textContent = 'This acknowledgement is mandatory once every quarter before accessing the employee portal.';
...
function initTabs() {
  ...
  const preferred = currentAccessRole() === 'sysadmin' || currentAccessRole() === 'org_admin'
    ? $$('.admin-tab').find(btn => btn.dataset.tab === 'tabTenancy')
    : ($$('.admin-tab').find(btn => btn.dataset.tab === 'tabEmployees') || $$('.admin-tab')[0]);
  (preferred && !preferred.classList.contains('hidden') ? preferred : $$('.admin-tab').find(btn => !btn.classList.contains('hidden')))?.click();
}
...
function bindEvents() {
  ...
  $('empOrganizationId')?.addEventListener('change', renderEmployeeHospitalSelect);
  bindTenancyEvents();

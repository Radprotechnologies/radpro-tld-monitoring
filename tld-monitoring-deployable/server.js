...
try {
  ...
  console.error('\nRadpro TLD Monitoring requires Node.js 22.5+ for this Windows-friendly build.');
...
// transaction() helpers, so we provide small equivalents here.
...
app.get('/health', (_req, res) => res.json({ ok: true, app: 'radpro-tld-monitoring', time: nowISO() }));
...
app.post('/api/organizations', requireAuth, requireRadproOrOrgAdmin, (req, res) => {
  ...
  const ts = nowISO();
  const id = makeId('org');
  const code = uniqueTableCode('organizations', req.body.code || name || id);
...
app.put('/api/organizations/:id', requireAuth, requireRadproOrOrgAdmin, (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'Organization name is required' });
  const code = uniqueTableCode('organizations', req.body.code || row.code || name, row.id);
  const duplicate = db.prepare('SELECT id FROM organizations WHERE code=? AND id<>?').get(code, row.id);
  if (duplicate) return res.status(409).json({ error: 'Organization code already exists' });
...
app.delete('/api/organizations/:id', requireAuth, requireRadproOrOrgAdmin, (req, res) => {
  ...
  const row = db.prepare('SELECT * FROM organizations WHERE id=?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Organization not found' });
  const hospitals = db.prepare('SELECT COUNT(*) AS n FROM hospitals WHERE organizationId=?').get(row.id).n;
  const users = db.prepare('SELECT COUNT(*) AS n FROM employees WHERE organizationId=?').get(row.id).n;
  if (hospitals > 0 || users > 0) return res.status(400).json({ error: 'Organization has hospitals/users. Deactivate it or delete/reassign hospitals/users first.' });
...
app.put('/api/hospitals/:id', requireAuth, requireRadproOrOrgAdmin, (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'Hospital / institute name is required' });
  const code = uniqueTableCode('hospitals', req.body.code || h.code || buildHospitalCode(org, name, req.body.city || h.city || ''), h.id);
  const duplicate = db.prepare('SELECT id FROM hospitals WHERE code=? AND id<>?').get(code, h.id);
  if (duplicate) return res.status(409).json({ error: 'Hospital code already exists' });
...
app.delete('/api/hospitals/:id', requireAuth, requireRadproOrOrgAdmin, (req, res) => {
  ...
  const n = db.prepare('SELECT COUNT(*) AS n FROM employees WHERE hospitalId=?').get(h.id).n;
  if (n > 0) return res.status(400).json({ error: 'Hospital has users. Disable/reassign users before deleting.' });
...
app.get('/api/doses/periods', requireAuth, (req, res) => {
  const rows = userCanReadAudit(req.user)
    ? db.prepare(`
        SELECT period, quarter, COUNT(*) AS count, ROUND(SUM(hp10), 4) AS totalHp10, ROUND(AVG(hp10), 4) AS avgHp10
